...
describe('Тесты слайса ингредиентов', () => {
  ...
  it('Проверяет начальное состояние слайса', () => {
    const actualState = ingredientsSlice.reducer(undefined, { type: '' });
    expect(actualState).toEqual(initialState);
  });

  it('Проверяет работу селектора getSelectedIngredients', () => {
    const state = {
      ingredients: {
        ...initialState,
        ingredients: testIngredient
    ...
    expect(selectedIngredients).toEqual(testIngredient);
  });

  it('Проверяет работу селектора getLoadingStatus', () => {
    const state = {
      ingredients: {
        ...initialState,
        loading: true
